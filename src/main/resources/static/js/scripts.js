...
const URLSEARCH = "https://api.themoviedb.org/3/search/tv";
 ...
 //Shows data from external api for index.html
...
async function populateGrid() {
  ...
  gridSection.innerHTML = results.map(result => `
    <div class="card animGrid">
      ...
      <div class="card-body">
        <h5 class="card-titulo" style="visibility: hidden">${result.name}</h5> 
      </div>
    </div>
  `).join('');

  // Add event listeners to toggle visibility of card titulo on hover
  const cards = document.querySelectorAll('.card');
  cards.forEach(card => {
    card.addEventListener('mouseenter', () => {
      card.querySelector('.card-titulo').style.visibility = 'visible';
    });
    card.addEventListener('mouseleave', () => {
      card.querySelector('.card-titulo').style.visibility = 'hidden';
    });
  });
}
